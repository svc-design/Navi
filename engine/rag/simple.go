...
func embedQuery(q string) []float64 {
	...
	n := 0.0
	for _, x := range out { n += x*x }
	n = math.Sqrt(n)
	if n == 0 { n = 1 }
	for i := range out { out[i] /= n }
	return out
}
...
func hash(s string) uint64 {
	...
	for i := 0; i < len(s); i++ { h ^= uint64(s[i]); h *= FNV64 }
	return h
}
...
func Run(question string) Answer {
	...
	// score
	type pair struct{ s float64; idx int }
	...
	for i := 0; i < len(ps); i++ {
		for j := i+1; j < len(ps); j++ {
			if ps[j].s > ps[i].s { ps[i], ps[j] = ps[j], ps[i] }
		}
	}
	if len(ps) > k { ps = ps[:k] }
	snips := []Snippet{}
